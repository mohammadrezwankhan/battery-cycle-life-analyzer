...
def test_load_cycle_data_long_form_multi_cell_with_metadata_and_envelope(tmp_path: Path):
    ...
    csv_path.write_text(
        ...
        "timestamp_iso,depth_of_discharge,energy_throughput_wh,duty_cycle_profile,protocol,source\n"
        "LFP-A,2,0.997,LFP,25,0.5,1,2026-07-01T00:00:00Z,0.8,1200,continuous,CC,lab\n"
        "LFP-A,1,1.00,LFP,23,0.5,1,2026-07-01T00:01:00Z,0.8,1000,continuous,CC,lab\n"
        "NMC-B,1,1.01,NMC,35,1.0,2,2026-07-01T00:02:00Z,0.9,1500,partial_cycles,CC,test\n"
        "NMC-B,3,0.99,NMC,36,1.1,2,2026-07-01T00:03:00Z,0.9,1510,partial_cycles,CC,test\n"
    )
    ...
    assert all(row["duty_cycle_profile"] in {"continuous", "partial_cycles"}
